...
Ez a cikk a teljes kioszkkonfigurációnak az egyes eszközökre jellemző aspektusait HoloLens be. A különböző típusú számítógép-alapú Windows és konfigurálásukról a Kioszkok és digitális aláírások konfigurálása asztali kiadásokon Windows [általános információkat.](/windows/configuration/kiosk-methods)  

> [!IMPORTANT]  
> A kioszkmód határozza meg, hogy mely alkalmazások érhetők el, amikor egy felhasználó bejelentkezik az eszközre. A kioszkmód azonban nem biztonsági módszer. Nem akadályozhatja meg, hogy az "engedélyezett" alkalmazások egy másik, nem engedélyezett alkalmazást is megnyithatnak. Az alkalmazások vagy folyamatok megnyitásának letiltása érdekében használja az Windows Defender [(WDAC) CSP-t](/windows/client-management/mdm/applicationcontrol-csp) a megfelelő szabályzatok létrehozásához.
>
> További információ a Microsoft-szolgáltatások, hogy a felhasználók a 2 HoloLens által használt magas szintű biztonságot nyújtson, további információ az Állapotelválasztás és -elkülönítés – [Defender-védelmi](security-state-separation-isolation.md#defender-protections)funkciókról. Vagy megtudhatja, hogyan engedélyezheti vagy tilthatja Windows PowerShell [wdac](/mem/intune/configuration/custom-profile-hololens)és a HoloLens 2 eszközön az Microsoft Intune.
...
-   A Kioszkkonfiguráció és a Felhasználói bejelentkezés **típusa** (a kioszkba bejelentkező felhasználók) és az Alkalmazások kiválasztása után az Eszközkonfigurációt továbbra is hozzá kell rendelni egy csoporthoz. A Hozzárendelt csoport(ök) határozza meg, hogy mely eszközök kapják meg a kioszkeszköz konfigurációját, azonban nem lép interakcióba a kioszk engedélyezése esetén. 
...
|Üzembe helyezés a Azure Active Directory (Azure AD) használatával  | Nem szükséges            | Nem szükséges                   | Kötelező  |
...
Ha a kioszkmódot egy másik MDM Microsoft Intune vagy más MDM-Microsoft Intune is használni, kövesse az alábbi lépéseket.
...
Az MDM-rendszert beállíthatja úgy, hogy automatikusan regisztrálja a HoloLens, amikor a felhasználó először jelentkezik be, vagy hogy a felhasználók manuálisan regisztrálják az eszközöket. Az eszközöket az Azure AD-tartományhoz is hozzá kell rendelni, és hozzá kell rendelni a megfelelő csoportokhoz.

Az eszközök regisztrálásával kapcsolatos további információkért lásd: Regisztráció HoloLens az [MDM-be](hololens-enroll-mdm.md) és az Intune regisztrációs [módszereivel Windows eszközök esetében.](/mem/intune/enrollment/windows-enrollment-methods)
...
A kioszkkonfigurációs profil létrehozásáról további információt az Intune-nal dedikált [kioszkként](/intune/configuration/kiosk-settings)való futtatás Windows 10 Windows Holographic for Business eszközbeállításokat.
...
- Az egyalkalmazásos kioszkok Intune-ban elérhető beállításaival kapcsolatos további információkért lásd: Egy teljes képernyős alkalmazás teljes [képernyős kioszk](/intune/configuration/kiosk-settings-holographic#single-full-screen-app-kiosks)
...
1. Válassza **a Felhasználói bejelentkezés** típusa Helyi felhasználói fiók lehetőséget, majd adja meg annak a helyi (eszköz-) fióknak vagy Microsoft-fióknak (MSA) a felhasználónevét, amely bejelentkezhet a  >  kioszkba.
   > [!NOTE]  
...
1. Válassza **a Felhasználói bejelentkezés típusa** Azure  >  **AD-felhasználó** vagy -csoport vagy a HoloLens bejelentkezési típus lehetőséget, majd adjon hozzá egy vagy több felhasználói csoportot   >  vagy fiókot.  
...
A **kioszkkonfigurációs** profil Hozzárendelések lapján beállíthatja, hogy hol szeretné telepíteni a kioszkkonfigurációt. A legegyszerűbb esetben a kioszkkonfigurációs profilt olyan csoporthoz kell hozzárendelni, amely az HoloLens eszközt fogja tartalmazni, amikor az eszköz regisztrál az MDM-be.
...
1. Várjon, amíg a kioszkkonfigurációs profil részét képezi az alkalmazások letöltése és telepítése. Emellett várja meg a szabályzatok alkalmazását.  
...
#### <a name="placeholder-start-layout-for-hololens"></a><a id="start-layout-for-hololens"></a>Helyőrző Kezdő elrendezése a HoloLens
...
- Windows Holographic, 20H2-es verzió – ha egy eszköz kioszkkonfigurációja a globális és az AAD-csoporttaghoz rendelt hozzáférés kombinációjából áll, ha az AAD-csoporttagság meghatározása sikertelen, a felhasználó a "start menüben semmi sem jelenik meg" jelenik meg.
