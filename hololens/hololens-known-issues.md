...
Az alábbi lista a különböző eszközök ismert HoloLens sorolja fel. Először ellenőrizze, hogy szokatlan viselkedést lát-e. A lista az új problémák felderített vagy jelentett frissítésekor, illetve a szoftverfrissítések későbbi megoldása során HoloLens frissül.
...
> - Ha a problémája blokkolja a problémát, a visszajelzés küldése mellett nyújtsa be [a támogatási kérést](https://aka.ms/hlsupport)is.
...
A OneDrive alkalmazás nem HoloLens támogatja a munkahelyi vagy iskolai fiókok automatikus kamerafeltöltését.
...
### <a name="unable-to-connect-and-deploy-to-hololens-through-visual-studio"></a>Nem lehet csatlakozni a virtuális géphez, HoloLens üzembe Visual Studio
...
Ha meg kell tudni, hogy az eszköz képes-e a frissítésre:
...
Emellett, ahogy az operációs rendszer minden kiadásában HoloLens, az FFU-rendszerképet is közzétettük a [Microsoft letöltőközpontjában.](https://aka.ms/hololensdownload/10.0.17763.380)

Ha nem szeretné frissíteni a frissítést, 2019. augusztus 29-től az UWP Microsoft Store új verzióját adták ki. Miután frissítette az Áruház frissített verzióját:
...
Ha az eszköz továbbra sem tud alkalmazásokat betölteni, a .NET Native Framework és a Runtime egy verzióját a letöltőközpontból töltheti be a következő lépésekkel:
...
1. Ellenőrizze, hogy az eszköz fel van-e oldva az eszközön.  Ha ezt még nem tette meg, tekintse meg a Using the Windows Eszközportál for instructions [(A](/windows/mixed-reality/using-the-windows-device-portal) Windows Eszközportál használata) útmutatót.
