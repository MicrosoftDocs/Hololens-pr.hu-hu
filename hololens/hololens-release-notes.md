...
ms.locfileid: "122859442"
---
# <a name="hololens-2-release-notes"></a>HoloLens 2. kiadási megjegyzések

Annak érdekében, hogy hatékony felhasználói élményt nyújtson a HoloLens, továbbra is kiadjuk a funkciókat, a hibákat és a biztonsági frissítéseket. Ezen az oldalon láthatja az egyes HoloLens újdonságokat. A 2. HoloLens frissítésének legújabb verziójához [](hololens-update-hololens.md#check-for-updates-and-manually-update) ellenőrizheti a frissítéseket, és manuálisan frissítheti, vagy a teljes flash frissítést (FFU) is lekértve az Advanced Recovery Companion (Speciális helyreállítás-kísérő) funkcióval frissítheti az [eszközt.](hololens-recovery.md#clean-reflash-the-device) A [letöltés](https://aka.ms/hololens2download) naprakész, és a legújabb általánosan elérhető buildet biztosítja.
...
- Eszközportál továbbfejlesztett metódusokkal értesíti az ügyfelet, Fájlkezelő problémákba ütközik a zárolt fájlok megnyitásakor.
- A fájlfeltöltés, -letöltés, -átnevezés és -törlés mostantól ki van javítva, ha https-t használ minden támogatott böngészőben.
- Kijavítva a hiba, amely miatt Wi-Fi proxy nem menthető, amikor Wi-Fi tulajdonságok felhasználói felülete Gépház **-> Network & Internet -> Status -> Properties** ről indul.
...
- A Gépház lapon megjelenő állapotüzenetben megjelenik a függőben lévő fájlok száma (vagy az "OneDrive naprakész" szöveg jelenik meg, ha az összes függőben lévő fájl fel lett töltve).
...
>
> A holografikus Windows 21H1 verziójának bevezetésével a **Holographic 1903-as** Windows havi karbantartási frissítéseit fontlunk meg. Ez lehetővé teszi, hogy a legújabb kiadásokra összpontosítsunk, és továbbra is értékes fejlesztéseket biztosítsunk.
...
[Pöccintsen a begépelhez](#swipe-to-type) | Az ujjlenyomata hegyével "pöccintsen" szavakat a holografikus billentyűzeten. | Végfelhasználó |
...
[Továbbfejlesztett kioszkmódú hibák osztása](#kiosk-mode-behavior-changes-for-handling-of-failures) | A kioszkmód az üres Start menü előtt keres globális hozzárendelt hozzáférést. | Rendszergazdai |
...
Az új Microsoft Edge a 2. HoloLens böngésző-szabályzatok sokkal szélesebb skáláját kínálja a rendszergazdáknak, mint ami korábban a régi Microsoft Edge.

Az új szabályzatbeállítások kezelésével kapcsolatos további információkért íme néhány hasznos Microsoft Edge:

- [Konfigurálja Microsoft Edge házirend-beállításokat a Microsoft Intune](/deployedge/configure-edge-with-intune)
...
Mivel az új Microsoft Edge egy natív Win32-alkalmazás, amely egy új UWP-adapterréteggel rendelkezik, amely lehetővé teszi, hogy csak UWP-eszközökön, például az HoloLens 2-n fusson, előfordulhat, hogy egyes funkciók nem lesznek azonnal elérhetők. A következő hónapokban új forgatókönyveket és funkciókat fogunk támogatni, ezért ebben a térben naprakész információkat is keres.
...
A Microsoft Edge három előzetes csatornát tesz elérhetővé az Edge Insider-közösség számára: Béta, Dev és Canary. Az előzetes verziójú csatorna telepítése nem távolítja el a Microsoft Edge kiadását a HoloLens 2-es verziójáról, és egyszerre több is telepíthető. 
...
**Telepítés számítógéppel Windows Eszközportál [(ehhez](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal#setting-up-hololens-to-use-windows-device-portal) engedélyezni kell a fejlesztői módot a 2. HoloLens)**
  ...
  1. A [Windows Eszközportál](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal#installing-an-app) .msix fájl telepítéséhez használja a számítógépen a HoloLens 2.
  1. A sikeres telepítés után a Microsoft Edge Beta, Dev vagy Canary külön bejegyzésként Minden alkalmazás **a** Start menü.
...
1. Amikor először próbálja elindítani a 360 Viewert egy adott tartományon, a böngésző jóváhagyást kér a modern nézetbe való belépéshez. Válassza az **Allow (Engedélyezése) lehetőséget.**
...
Ossza meg a visszajelzéseket és  a hibákat a csapatunkkal az új e-mail-üzenet visszajelzés küldése Microsoft Edge.

### <a name="new-settings-app"></a>Új Gépház alkalmazás

Ebben a kiadásban a Gépház új verzióját vezetjük be. Az új Gépház alkalmazás új funkciókat és kibővített beállításokat tartalmaz a HoloLens 2-hez a következő területeken: Hang, Power & alvó üzemmód, Network & Internet, Alkalmazások, Fiókok, Könnyű kezelés stb.
...
- Rendszer > Power & alvó üzemmódban: válassza ki, hogy az eszköz mikor alvó üzemmódba ússzanak egy bizonyos tétlenség után.
...
Ezzel az új beállítással alternatív színprofilt választhat a 2. HoloLens megjelenítéséhez. Ez segíthet a színek pontosabb megjelenítésében, különösen alacsonyabb megjelenítési fényerejénél. A színkorrektálás megjelenítése a Gépház a Rendszer és > lapon található.
...
6. Zárja be a menüt bárhol másra, vagy zárja be Start menü a Start kézmozdulattal.
...
> [!IMPORTANT]
> Az USB-mikrofon csatlakoztatása nem lesz automatikusan beállítva **bemeneti eszközként.** USB-C-kábelek csatlakoztatása esetén a felhasználók megfigyelhetik, hogy a mikrofon hanganyaga automatikusan át lesz irányítva a mikrofonhoz, de a HoloLens operációs rendszer a belső mikrofontömböt a többi bemeneti eszköz fölé rangsorítja. **USB-C mikrofon használata érdekében kövesse az alábbi lépéseket.**
...
Vegye figyelembe, hogy egyes USB-C-mikrofonok helytelenül jelentik magukat mikrofonként és *beszélőként* is. Ez a mikrofonnal, és nem a mikrofonnal HoloLens. Ha ezen mikrofonok valamelyikét csatlakoztatja a HoloLens, előfordulhat, hogy a hang elveszett. Szerencsére van egy egyszerű javítás.  
...
A jelen Windows kiadástól kezdve a kioszkmód vissza fog állni a globális kioszkkonfigurációhoz (ha van ilyen), ha az AAD-csoport kioszkmódjában hiba lép fel.
...
>
> Ezekben az újabb buildekben az egyik beállítási oldal elavult. A régi **Hálózati &-proxy** lap már nem érhető el  >   globális beállításként. Az új kapcsolatonkénti proxybeállítások a Network **& Wi-Fi** Properties (Internetes  >  **Wi-Fi-tulajdonságok)** vagy  >   a Network & Internet Ethernet Properties **(Internet Ethernet-tulajdonságok)** alatt  >    >  **találhatók.**
...
A Gépház alkalmazásban a felhasználó konfigurálhatja a [Fallback Diagnostics viselkedését.](hololens-diagnostic-logs.md) A beállítás Gépház alkalmazás **Adatvédelmi** hibák elhárítása  ->  **lapjára** navigálva konfigurálhatja ezt a beállítást.
...
Megoszthatja a dolgokat a Windows 10 eszközökkel, beleértve a számítógépeket és a 2 HoloLens eszközöket is. Kipróbálhatja a megosztott Gépház- és URL-címek számítógépen HoloLens  ->    ->   megosztásához. További részletekért olvassa el, hogyan oszthat meg dolgokat a közeli eszközökkel a [Windows 10.](https://support.microsoft.com/windows/share-things-with-nearby-devices-in-windows-10-0efbfe40-e3e2-581b-13f4-1a0e9936c2d9)
...
A Gépház alkalmazás korábbi hibaelhárítói mellett egy új hibaelhárítót is hozzáadtunk az új Gépház operációsrendszer-frissítésekhez. Lépjen a **Gépház**  ->  **Update Security &amp; hibaelhárítási**  >  **Windows,** és válassza  >  **az** Indítás **lehetőséget.** Ez lehetővé teszi a nyomkövetések gyűjtését az operációsrendszer-frissítésekkel kapcsolatos probléma reprodukálása során, így jobb hibaelhárítást tud nyújtani az IT-hez vagy a támogatáshoz.
...
- A hibaelhárításhoz vagy diagnosztikára van szükség a Csatlakoztatott gyorsítótár-kiszolgálón, vagy nyomkövetést kell gyűjteni a HoloLens-on HoloLens-on Gépház  >  **Update & Security Troubleshooting** Windows Update (Frissítés & biztonsági  >   **hibaelhárítása)**  >   **Windows frissítésen keresztül.**
...
✔️ Ha korábban blokkolta a Microsoft Edge WDAC-n keresztül, frissítenie kell a WDAC-szabályzatot. Tekintse át az alábbiakat, és használja a megadott mintakódot.
...
- Azok az ügyfelek, akik az AdvancedPhotoCapture vagy a LowLagPhotoCapture használatával rögzítik HoloLens 2.-es képpel készített fényképeket, a fénykép rögzítése után legfeljebb 3 másodperccel tudják lekérni a kamerát.
...
| [Automatikus kiépítés felhasználói felület használata nélkül](hololens-release-notes.md#automatic-provisioning-without-using-ui) | Az automatikus indítás és az automatikus megerősítés kombinálása. |
...
1.  Csatlakozás 2. HoloLens számítógéphez.
...
1. Lépjen a **Gépház App > Update and Security > Certificates (Tanúsítványok) lapra.**
...
[TenantLockdown (Bérlői zárolás)](/windows/client-management/mdm/tenantlockdown-csp) A CSP lehetővé HoloLens, hogy a 2. tanúsítvány kizárólag autopilot használatával legyen MDM-regisztrációhoz kötve. Miután a TenantLockdown CSP RequireNetworkInOOBE csomópontja true (igaz) vagy false (kezdetben beállított) értékre van állítva HoloLens 2-es verzióban, ez az érték megmarad az eszközön az újra flash() vagy az os updates (operációs rendszer frissítése) ellenére is. 

Ha HoloLens 2. HoloLens TenantLockdown CSP-k RequireNetworkInOOBE csomópontja igazra van állítva, az OOBE határozatlan ideig vár az Autopilot-profil sikeres letöltésére és alkalmazásra a hálózati kapcsolat után. 
...
Korábban, amikor sikertelen volt a kioszkmód alkalmazása, HoloLens a start menüben az összes alkalmazás megjelenik. A Windows Holographic 20H2-es verziójában hibák esetén nem jelennek meg alkalmazások a Start menüben az alábbiak szerint:
...
- Eszközkezelési lehetőségek kifejezetten HoloLens eszközkezeléshez létrehozott eszközökre. 
...
### <a name="enabled-settings-page-visibility-for-hololens-2"></a>Az Gépház oldal láthatósága engedélyezve HoloLens 2. HoloLens esetén
- A felhasználói felület nagyobb mértékű vezérlése Gépház alkalmazásában, amely nem mindig mutat korlátozott számú oldalt.

Most már engedélyeztünk egy olyan szabályzatot, amely lehetővé teszi a rendszergazdák számára, hogy megakadályozzák a System Gépház-alkalmazás adott lapjainak láthatóvá vagy hozzáférhetővé tételeit, vagy hogy ezt a megadott oldalak kivételével az összes oldalon megtenzék. Ha meg szeretne ismerkedni a funkció teljes testreszabásának mikéntjére, kattintson az alábbi hivatkozásra.
...
-   A HoloLens (1. generációs) kutatási módban elérhető érzékelők mellett mostantól IMU-érzékelői hozzáférést is biztosítunk, beleértve a gyorsulásmérőt, a groscope-t és a kilométermérőt is.
...
- A 2. Windows Eszközportál https-továbbítás HoloLens ával kapcsolatos probléma elhárítása.
...
- Hozzá van adva egy beállítás **a Gépház** System Hologramok, amely lehetővé teszi a felhasználók számára, hogy automatikusan eltávolítsanak minden hologramot Mixed Reality az eszköz leállított  >    >   eszközéről.
...
- Kijavítottunk egy problémát, amely HoloLens, amelyek a képpontformátumukat úgy módosítják, hogy feketét renderelnek a HoloLens Emulator.
...
A HoloLens 2020. májusi fő szoftverfrissítése, a *Windows Holographic 2004-es* verziója számos izgalmas új képességet tartalmaz, többek között az Windows Autopilot támogatását, az alkalmazás sötét üzemmódját, az USB Ethernet-támogatást az 5G/LTE-elérési pontokhoz. A legújabb kiadásra való frissítéshez nyissa meg a **Gépház** alkalmazást, nyissa meg az Update & Security (Biztonsági frissítések frissítése) lehetőséget, és válassza a   Frissítések keresése  **** ****   gombot. 
...
Windows Az Autopilot for HoloLens 2 lehetővé teszi, hogy az eszköz értékesítési csatornája előre regisztrálja HoloLens intune-bérlőjébe. Amikor az eszközök megérkeznek, készen állnak a bérlőn megosztott eszközként való önálló üzembe helyezésre. Az öntelepítés előnyeinek kihasznál érdekében az eszköznek a telepítés első képernyőjén csatlakoznia kell egy hálózathoz egy USB-C-to-Ethernet kapcsolaton keresztül.
...
A kiépítési csomagok segítségével konfigurációs HoloLens állíthatja be a konfigurációt ahelyett, hogy a HoloLens lenne. Korábban a kiépítési csomagokat át kellett másolni a HoloLens memóriába. Ezek mostantól USB-meghajtón is használhatók, így könnyebben újra felhasználhatók több HoloLens eszközön, és az eszközöket párhuzamosan is kiépítheti. A kiépítési csomagok mostantól támogatják az eszközkezelésbe való regisztrációhoz szükséges mezőt is, így a kiépítés után nincs szükség manuális beállításra.

A kipróbálhoz:

1. Töltse le a Windows Configuration Designer legújabb verzióját a Windows a számítógépére.
...
- **A stabilitás rámutatása:** A rendszer most már nem állja meg az indexavat, amikor a torkok eltoltják. Ez a módosítás javítja a pontosságot a gombok leküldésekor, a begépelésekor, a tartalom görgetésekor és így tovább. 
...
- Kijavítottuk a holografikus alkalmazások indításkor szüneteltetett állapotban való elakadásával kapcsolatos problémákat, Start menü megnyitották és lezárták.
