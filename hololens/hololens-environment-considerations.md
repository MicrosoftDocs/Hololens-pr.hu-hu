...
A tér különböző típusú világítása is befolyásolhatja a követést. A villanykörték olyan pulzust adnak, amely a áram-áramot átsziratja – ha az AC-gyakoriság 50 Hz, akkor a fényjel 50 Hz-nél. Ez az emberi élet nem vette észre. A HoloLens 30 kamera azonban látja ezeket a változásokat – egyes képkockák jól begyújtottak, némelyik rosszul lesz begyújtva, és néhány túl lesz fedve, ahogy a kamera megpróbálja kompenzálni a fény pulzusát.  

Az Egyesült Államokban az elektromos áram szabványa 60 Hz, ezért a villanykörte pulzusát HoloLens HoloLens képkocka-rátával – a 60 Hz-es pulzusok az HoloLens 30 FT-os képkocka-értékéhez igazodnak. Számos országban azonban 50 Hz-es AC-gyakorisági szabvány van, ami azt jelenti, hogy HoloLens egyes képkockák az pulzusok során lesznek felveve, mások viszont nem. Az európai fényviszonyok különösen is problémákat okozhatnak.  
...
Hasonlóan ahhoz, ahogy az emberek nem tudnak jól koncentrálni a szemhez közel lévő objektumokra, HoloLens, amikor objektumok állnak a kameráihoz közel. Ha egy objektum túl közel van ahhoz, hogy mindkét kamerával látható legyen, vagy ha egy objektum blokkol egy kamerát, az eszköz sokkal több problémát fog okozhatni az objektum nyomon követése során.  
...
Amikor új szóközt ad meg (vagy betölt egy meglévőt), egy, a térben elterülő hálós ábrát fog látni. Ez azt jelenti, hogy az eszköz leképezi a környezetet. Bár a HoloLens idővel teret tanulnak, tippek és trükkök is rendelkezésre állnak a terek leképezésében.

## <a name="environment-management"></a>A környezet kezelése

Két beállítás van, amelyek lehetővé teszik, hogy a felhasználók "megtisztítsák" a hologramokat, és HoloLens" a szóközt. A beállítások **Hologramok** és környezetekben léteznek, a második beállítás pedig  az Adatvédelem alatt is megjelenik a beállításalkalmazásban.  

1. **Törölje a közeli hologramokat.** Ha ezt a beállítást választja, HoloLens törli az összes rögzített hologramot és az eszköz "aktuális térben" tárolt térképi adatait. Egy új térképszakasz jön létre és lesz tárolva az adatbázisában, ha a hologramok ismét ugyanoda kerülnek.

1. **Törölje az összes hologramot.** Ezzel a beállítással a HoloLens törli az ÖSSZES térképadatot és a rögzített hologramokat a teljes terek adatbázisában. A rendszer nem fogja újra felfedezni a hologramokat, és minden hologramot újonnan kell elhelyezni az adatbázis térképszakaszainak tárolására.

## <a name="hologram-quality"></a>Hologram minősége

Hologramok a környezetben – magas, alacsony és körül lehet helyezni – de egy holografikus képkockán keresztül fogja látni őket, amely [a](/windows/mixed-reality/holographic-frame) tekintete előtt áll. A legjobb nézet érdekében állítsa be az eszközt úgy, hogy az a teljes keretet láthatóvá tegye. Ne aggódjon, ha körbe járja a környezetét, és felfedezi a felfedezést!

Ahhoz, hogy [a hologramok](/windows/mixed-reality/hologram) szépnek, egyértelműnek és stabilnak nézzenek, HoloLens csak Ön számára kell besziratálni a hologramokat. A folyamat végigvezeti HoloLens első beállításakor. Később, ha a hologramok nem jól néznek ki, vagy sok hibát lát, módosításokat is lehet végezni.

Ha nem tudja leképezni a helyeket, próbálja meg törölni a közeli hologramokat, majd újra leképezni a területet.

### <a name="calibration"></a>Kalibrációs

Ha a hologramok jitterynek vagy rezdülnek, vagy ha problémája van a hologramok elhelyezési problémáival, az első dolog, amit meg kell próbálnia, az [aHatomság alkalmazás.](hololens-calibration.md) Ez az alkalmazás abban is segíthet, ha bármilyen kellemetlenséget tapasztal a HoloLens.
...
Az alábbi irányelveket kell követni az eszköz használata esetén:
